...
type Message = {
  role: "user" | "assistant";
  content: string; // ✅ Fixed: was "string" (string literal type)
};
...
const Chat = () => {
  ...
  const handleDeleteChats = async () => {
    try {
      ...
      console.log(error);
      toast.error("Chat Deletion Failed", { id: "deletechats" }); // ✅ Fixed: was toast.loading
    }
  };
    ...
    >
      ...
      >
        ...
        >
          ...
          >
            {/* ✅ Fixed: safe access for users with no last name */}
            {auth?.user?.name[0]}
            {auth?.user?.name.split(" ")[1]?.[0]}
          </Avatar>
          <Typography sx={{ mx: "auto", fontFamily: "work sans" }}>
          ...
          <Button
            onClick={handleDeleteChats} // ✅ Fixed: was onclick (lowercase)
            sx={{
              width: "200px",
              my: "auto",
              color: "white",
